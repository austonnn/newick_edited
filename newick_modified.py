...
def _read_newick_from_string(nw, root_node, matcher, formatcode):
    ...
    for chunk in split_ext(nw, "(")[1:]:
        ...
        #subchunks = [ch.strip() for ch in split_ext(chunk, ",")]
...
def _read_node_data(subnw, current_node, node_type, matcher, formatcode):
    ...
    if data:
        ...
        raise NewickError("Unexpected newick format '%s' " %subnw) #%subnw[0:50]
    return
...
def split_ext(text, splitter):
    tmp = text.split("'")
    i = 1
    result = []
    for item in tmp[0].split(splitter):
        result.append(item)    
    while i < len(tmp):
        
        if i %2 == 0:
            result[-1] += "'"
            result[-1] += tmp[i].split(splitter)[0]            
            for item in tmp[i].split(splitter)[1:]:
                result.append(item)
